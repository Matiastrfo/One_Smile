...
const STATUS_CONFIG = {
  PENDING:   { label: "Pendiente",  icon: Clock,        cls: "bg-slate-100 dark:bg-slate-800/60 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-700",        dot: "bg-slate-400" },
  CONFIRMED: { label: "Confirmado", icon: CheckCircle2, cls: "bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-800",           dot: "bg-blue-500" },
  ATTENDED:  { label: "Atendido",   icon: CheckCircle2, cls: "bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800",  dot: "bg-emerald-500" },
  ABSENT:    { label: "Ausente",    icon: XCircle,      cls: "bg-rose-100 dark:bg-rose-900/40 text-rose-700 dark:text-rose-300 border-rose-200 dark:border-rose-800",           dot: "bg-rose-500" },
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {/* Nota */}
                    {appt.notes && (
                      <div className="mt-1 inline-flex items-center gap-1 text-[11px] text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded-md px-2 py-0.5 max-w-full">
                  ...
                  <div className="flex items-center gap-0.5 shrink-0">
                    <button
                      ...
                      className="p-1.5 rounded-lg text-muted-foreground hover:text-amber-500 hover:bg-amber-50 dark:hover:bg-amber-950/40 transition-colors"
                      title="Nota"
                    >
                      ...
                      {openMenu === appt.id && (
                        <div className="absolute right-0 top-8 z-50 bg-card border border-border/60 rounded-xl shadow-lg py-1 w-44" onClick={e => e.stopPropagation()}>
                          ...
                          <button onClick={() => { onDelete(appt.id!); setOpenMenu(null); }}
                            className="w-full flex items-center gap-2 px-3 py-2 text-xs hover:bg-rose-50 dark:hover:bg-rose-950/40 text-rose-600 dark:text-rose-400 transition-colors text-left">
